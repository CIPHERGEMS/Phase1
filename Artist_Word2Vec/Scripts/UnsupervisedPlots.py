# -*- coding: utf-8 -*-
"""
Created on Sat Oct 10 23:59:07 2020
...
results_path = homebase_path / "Desktop" / "glove_results"
...
for year in yearsloop:
    for embed_lib in lib_list:
        ...
        for num_first_words_2_use in [50, 100, 150, 200, 250, 300]:   # keep 100 first words     
            ...
            for i,artist_path in enumerate(artist_paths):
                artist_name = (Path(artist_path).name).split('.')[0]
                dict_artist_2_number[artist_name] = i
                ...
                zipped = zip(list_tracks, list_artist_name, list_y_val)
                df_artist_temp = pd.DataFrame(zipped)
                
                list_df_artist_tracks.append(df_artist_temp)
                list_num_artist_tracks.append(len(df_artist_temp))

            top_sort_index = np.argsort(list_num_artist_tracks)[::-1][:10]
            for df_idx in top_sort_index:
                df_artist_temp = list_df_artist_tracks[df_idx]
                if len(df_artist_temp) != 0:
                    df_artist_temp.columns = ['text','artist','label']                    
                    if df_artist_total is not None:
                        df_artist_total = pd.concat([df_artist_total, df_artist_temp])
                    else:
                        df_artist_total = df_artist_temp  
    ...
    #%%
            ...
            tfidf_df.columns = tfidf_cols +[ 'artist', 'label']
